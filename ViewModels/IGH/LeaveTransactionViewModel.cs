...
namespace ptc_IGH_Sys.ViewModels.IGH
{
    public class LeaveTransactionViewModel
    {
        ...
        public DateTime? LeaveDate { get; set; }   // made nullable

        [Required(ErrorMessage = "Leave Count is required")]
        [Range(0.5, 365, ErrorMessage = "Leave Count must be between 0.5 and 365")]
        public decimal? LeaveCount { get; set; }   // made nullable

        [Required(ErrorMessage = "Leave Type is required")]
        [StringLength(50)]
        public string? LeaveType { get; set; }     // made nullable

        public int? LeaveMonth { get; set; }       // made nullable
        ...
        public void Update(ptc_IGH_Sys.Models.IGH.IGH_Leave_Transaction target)
        {
            ...
            target.Leave_Date = this.LeaveDate ?? DateTime.MinValue; // fallback if null
            target.Leave_Count = this.LeaveCount ?? 0;                // fallback if null
            target.Leave_Type = this.LeaveType?.ToUpper();
            target.Leave_Month = this.LeaveDate?.Month ?? 0;
            target.Remarks = this.Remarks ?? string.Empty;
        }
    }
